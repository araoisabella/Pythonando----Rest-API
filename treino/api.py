...
def aula_realizada(request, aula_realizada: AulaRealizadaSchema):
    ...
    if qtd <= 0:
        raise HttpError(400, "Quantidade de aulas deve ser maior do que zero.")
...
def update_aluno(request, aluno_id: int, aluno_data: AlunosSchema):
    ...
    if int(idade.days/365) < 18 and aluno_data.dict()['faixa'] in ('A', 'R', 'M', 'P'):
        raise HttpError(400, "Menores de 18 anos não podem receber essa faixa.")

    # Alternativamente (para o if)
    #exclude_unset=True
    for attr, value in aluno_data.dict().items():
        # Para não aceitar valores vazios
        if value:
            # altera o atributo attr para value da instância aluno
